...
if "subgroup_count_v2" not in st.session_state:
    st.session_state.subgroup_count_v2 = 1
...
with b1:
    if st.button("+ Add subgroup"):
        ...
        st.rerun()
with b2:
    if st.button("− Remove last subgroup") and st.session_state.subgroup_count_v2 > 1:
        st.session_state.subgroup_count_v2 -= 1
        st.rerun()

subgroup_defs = []

for i in range(st.session_state.subgroup_count_v2):
    with st.expander(f"Subgroup {i+1}", expanded=True):
        name = st.text_input("Subgroup name", f"Subgroup {i+1}", key=f"name_v2_{i}")
...
if st.button("Generate subgroup fields"):
    ...
    st.session_state["generated_subgroups_v2"] = generated

generated_subgroups = st.session_state.get("generated_subgroups_v2", [])
